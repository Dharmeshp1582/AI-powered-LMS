  ...
  return (
    ...
    <div className='w-[100%] h-[70px] fixed px-[20px] py-[10px] flex items-center justify-between bg-[#00000047] z-10'>
      ...
      <div className='w-[30%] lg:flex items-center justify-center gap-4 hidden'>
        ...
        {/* User Avatar / Icon */}
        {!userData && (
          <IoPersonCircle onClick={()=> setShow(!show)} className='w-[50px] h-[50px] fill-white cursor-pointer' />
        ) }
         ...
         {userData?.photoUrl ? <img src={userData?.photoUrl} alt="" className='rounded-full object-cover border-2 border-black ' /> : <div className=' text-white flex items-center justify-center text-[20px]  bg-black border-white cursor-pointer'>
...
<span  onClick ={()=> navigate("/profile")} className='bg-black text-white px-[30px] py-[10px] rounded-2xl hover:bg-gray-600 '>My Profile</span>
<span className='bg-black text-white px-[30px] py-[10px] rounded-2xl hover:bg-gray-600' onClick={()=> navigate("/mycourses")}>My Courses</span>
        ...
        <RxHamburgerMenu className='w-[30px] h-[30px] lg:hidden fill-black text-white cursor-pointer' onClick={()=> setShowHam(!showHam)} />
        ...
        <RxCross2 color='white' className='w-[35px] h-[35px] fill-white absolute top-5 right-[4%] cursor-pointer' onClick={()=> setShowHam(!showHam)} />
         ...
         {userData && (
          <div className='w-[70px] h-[70px] rounded-full text-white flex items-center justify-center bg-black border-white cursor-pointer border-2 text-[20px]'>
            {userData.photoUrl ? <img src={userData?.photoUrl} alt="" className=' rounded-full object-cover border-2 border-black ' /> : <div className=' text-white flex items-center rounded-full justify-center text-[20px] border-2 bg-black border-white cursor-pointer w-[70px] h-[70px]'>
              {userData?.name?.slice(0, 1).toUpperCase()}
            </div>}
          ...
          <div onClick={()=> navigate("/profile")}  className='w-[180px] h-[60px] flex items-center justify-center border-2 border-white lg:border-black text-white bg-black lg:text-black rounded-[10px] text-[18px] font-light cursor-pointer'>
            My Profile
          </div>
        
        <div className='w-[180px] h-[60px] flex items-center justify-center border-2 border-white lg:border-black text-white bg-black lg:text-black rounded-[10px] text-[18px] font-light cursor-pointer' onClick={()=> navigate("/mycourses")}>
            My Courses
          </div>
